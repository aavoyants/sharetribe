module PaypalHelper
  ...
  def account_prepared_for_user?(user_id, community_id)
    ...
    personal_account_prepared?(m_account)
  end
  ...
  def account_prepared_for_community?(community_id)
    ...
    community_account_prepared?(m_account)
  end

  def personal_account_prepared?(m_account)
    permissions = m_account[:order_permission_state].or_else(:not_verified)
    billing_agreement = m_account[:billing_agreement_state].or_else(:not_verified)

    permissions == :verified && billing_agreement == :verified
  end

  def community_account_prepared?(m_account)
    permissions = m_account[:order_permission_state].or_else(:not_verified)
